...
function loadInitialData() {
    ...
    // Load portfolio value and day gain
...
function renderPortfolioValueChart() {
    ...
    if ( !container || portfolioValueHistory.length < 2) {
        if (container && portfolioValueHistory.length < 2) {
            container.innerHTML='<p style="text-align: center; color: #888; padding-top: 80px;">Portfolio value will be charted after multiple refreshes...</p>';
        }
...
function createLiveSection() {
    ...
    // Already set up via HTML, just refresh connection status
    connectToEventSource();
}
...
function refreshLiveSectorChart() {
    ...
    // Get holdings data
    if (typeof holdingsDetailData==='undefined'|| !Array.isArray(holdingsDetailData)) {
        container.innerHTML='<p style="text-align: center; color: #888; padding-top: 80px;">Holdings data not available</p>';
        return;
    }
    ...
    holdingsDetailData.forEach(function(holding) {
