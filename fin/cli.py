...
For more information, visit: https://github.com/akourk/fin
    ...
    parser.add_argument(
        "-q", "--quiet", action="store_true", help="Minimize output (only show errors)"
    )
    ...
    # Report options
    parser.add_argument(
        "--no-dashboard", action="store_true", help="Skip dashboard data generation"
    )
    ...
    # Cache options
    parser.add_argument(
        "--clear-cache", action="store_true", help="Clear all caches before processing"
    )

    parser.add_argument(
        "--no-cache", action="store_true", help="Disable cache usage (fetch all prices fresh)"
    )
