...
def check_large_price_discrepancies(holdings_df: pd.DataFrame, threshold_pct: float = 1000) -> List[QualityIssue]:
    """
...
def check_missing_cost_basis_filtered(df: pd.DataFrame, holdings_df: pd.DataFrame) -> List[QualityIssue]:
    """
...
def check_corporate_action_basis(df: pd.DataFrame, holdings_df: pd.DataFrame) -> List[QualityIssue]:
    ...
    for _, row in spinoffs.iterrows():
        ...
        if is_special:
            ...
            if holdings_df is not None and not holdings_df.empty:
                holding = holdings_df[
                    (holdings_df['Account'] == account) & 
                    (holdings_df['Symbol'] == symbol)
                ]
    ...
    for _, row in merger_dates.iterrows():
        ...
        has_stock = 'MergerOut' in event['Action'].values and any(event[event['Action'] == 'MergerOut']['Quantity'] > 0)
...
def run_quality_checks(df: pd.DataFrame, 
                      ...
                      dupes_removed: int = 0) -> Tuple[List[QualityIssue], Dict[str, int]]:
    """
    ...
    logger.info(f"Quality checks complete: {summary['errors']} errors, {summary['warnings']} warnings, {summary['info']} info")
...
def print_quality_report(issues: List[QualityIssue], summary: Dict[str, int]) -> None:
    ...
    # Summary
    print(f"\n{'='*60}")
    print(f"Total Issues: {summary['total']} ({summary['errors']} errors, {summary['warnings']} warnings, {summary['info']} info)")
    print("="*60)
