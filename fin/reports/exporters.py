...
from ..config import DASHBOARD_DATA_PATH, DATA_OUTPUT_PATH
...
def export_performance_data_js(performance_report: dict, filename: str = "performance_data.js"):
    ...
    export_data = {
        ...
        "accounts": (
            performance_report.get("accounts", pd.DataFrame()).to_dict(orient="records")
            if isinstance(performance_report.get("accounts"), pd.DataFrame)
            else []
        ),
        "sectors": (
            performance_report.get("sectors", pd.DataFrame()).to_dict(orient="records")
            if isinstance(performance_report.get("sectors"), pd.DataFrame)
            else []
        ),
        "assets": (
            performance_report.get("assets", pd.DataFrame()).to_dict(orient="records")
            if isinstance(performance_report.get("assets"), pd.DataFrame)
            else []
        ),
