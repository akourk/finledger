...
def calculate_net_cash_flows(
    df: pd.DataFrame, period_start: datetime, period_end: datetime
) -> float:
    """
...
def calculate_time_weighted_return(historical_results: list, df: pd.DataFrame) -> list:
    ...
    for i, result in enumerate(historical_results):
        ...
        if start_value + net_cash_flow / 2 > 0:
            period_return = (end_value - start_value - net_cash_flow) / (
                start_value + net_cash_flow / 2
            )
        else:
            period_return = 0.0
...
def calculate_cost_basis_changes(df: pd.DataFrame, price_cache: dict) -> list:
    ...
    for idx, row in df.iterrows():
        ...
        new_cost_basis = sum(
            sum(lot["remaining_cost"] for lot in position_lots) for position_lots in lots.values()
        )
        ...
        if prev_date is not None and date != prev_date and pending_delta != 0:
            ...
            sp500_price, _ = get_price_from_yfinance(
                sp500_symbol, date_str, price_cache, None, None
            )
            cost_basis_changes.append(
                {"date": prev_date, "delta": pending_delta, "sp500_price": sp500_price}
            )
            pending_delta = 0.0
...
def calculate_portfolio_cost_basis_history(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        ...
        if date != last_date:
            ...
            cost_basis_history.append({"Date": date, "TotalCostBasis": round(total_cost_basis, 2)})
            last_date = date
        else:
...
def calculate_historical_holdings(
    ...
    snapshot_dates = sorted(
        list(set([pd.to_datetime(d) for d in snapshot_dates if pd.to_datetime(d) <= today]))
    )
    ...
    print(
        f"Calculating historical holdings for {len(snapshot_dates)} dates across {len(accounts)} accounts..."
    )
    ...
    for snapshot_date in snapshot_dates:
        ...
        for idx in holdings.index:
            ...
            price, _ = get_price_from_yfinance(
                symbol, snapshot_date_str, price_cache, unavailable_ticker_cache, split_cache
            )
            ...
            for _, cash_row in include_cash.iterrows():
                ...
                if cash_account in accounts:
                    ...
                    if not cash_txns.empty:
                        ...
                        if cash_balance > 0:
                            account_values[cash_account] = (
                                account_values.get(cash_account, 0) + cash_balance
                            )
    ...
    for result in results:
        ...
        sp500_price, _ = get_price_from_yfinance(
            sp500_symbol, date_str, price_cache, unavailable_ticker_cache, split_cache
        )
