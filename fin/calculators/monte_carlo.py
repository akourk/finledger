...
def estimate_historical_stats(historical_df: pd.DataFrame) -> Dict[str, float]:
    ...
    if len(returns) < 30:
        ...
        return {"annual_return": 0.07, "annual_volatility": 0.15, "data_points": len(returns)}
...
def run_monte_carlo_simulation(
    ...
    yearly_values = {
        key: [round(values[i], 2) for i in yearly_indices] for key, values in percentiles.items()
    }
...
def run_scenario_analysis(
    starting_value: float, annual_contribution: float = 0, years: int = DEFAULT_YEARS
) -> Dict:
    """
