...
def calculate_account_performance(holdings_df: pd.DataFrame, price_cache: dict,
    ...
    accounts = asset_perf.groupby("Account").agg({
        "CurrentValue": "sum",
        "CostBasis": "sum"
    }).reset_index()
    ...
    for _, account_row in accounts.iterrows():
        ...
        result = {
            ...
            "TotalReturn": ((total_value - account_row["CostBasis"]) / account_row["CostBasis"] * 100) 
                          if account_row["CostBasis"] > 0 else 0,
...
def calculate_portfolio_performance(holdings_df: pd.DataFrame, price_cache: dict,
    ...
    result = {
        ...
        "TotalReturn": round(((total_value - total_cost) / total_cost * 100) if total_cost > 0 else 0, 2),
...
def calculate_sector_performance(holdings_df: pd.DataFrame, price_cache: dict,
    ...
    # Group by sector
    sectors = asset_perf.groupby("Sector").agg({
        "CurrentValue": "sum",
        "CostBasis": "sum"
    }).reset_index()
    ...
    for _, sector_row in sectors.iterrows():
        ...
        result = {
            ...
            "TotalReturn": ((total_value - sector_row["CostBasis"]) / sector_row["CostBasis"] * 100) 
                          if sector_row["CostBasis"] > 0 else 0,
...
def generate_performance_report(holdings_df: pd.DataFrame, price_cache: dict,
    ...
    print(f"   Calculated performance for {len(asset_perf)} assets across {len(periods)} time periods")
