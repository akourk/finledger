...
def get_cumulative_split_multiplier(
    ticker: str, from_date: str, to_date: str, split_cache: Dict
) -> float:
    """
...
def detect_unadjusted_transactions(df: pd.DataFrame, split_cache: Dict) -> List[Dict[str, Any]]:
    """
...
def calculate_spinoff_basis_allocation(
    spinoff: SpinOffAllocation, parent_cost_basis: float, parent_shares: float
) -> Tuple[float, float]:
    """
...
def lookup_spinoff_fmv(
    parent_symbol: str, spinoff_symbol: str, distribution_date: str, price_cache: Dict
) -> Tuple[float, float]:
    """
...
def estimate_spinoff_allocation(
    spinoff_symbol: str, distribution_date: str, parent_symbol: str = None, price_cache: Dict = None
) -> SpinOffAllocation:
    """
    ...
    if exchange_ratio == 0 and cash_per_share > 0:
        ...
        recognized_gain = min(
            ...
            - (
                per_share_basis
                * acquired_shares
                * (cash_per_share / (cash_per_share + exchange_ratio))
            ),
        )
        ...
        report_data.append(
            {
                ...
                "Details": (
                    action.description or f"Ratio: {action.split_ratio}"
...
def check_unprocessed_mergers(df: pd.DataFrame, holdings_df: pd.DataFrame) -> List[Dict[str, Any]]:
    """
...
def check_corporate_actions(df: pd.DataFrame, holdings_df: pd.DataFrame = None) -> List[Any]:
    """
