...
def get_price_from_yfinance(
    ...
    try:
        ...
        if hist.empty:
            logger.warning(
                f"No price data found for {ticker} around {date_str} - caching as unavailable"
            )
            print(
                f"Warning: No price data found for {ticker} around {date_str} (will skip in future)"
            )
            ...
            if unavailable_cache is not None:
                ...
                logger.debug(
                    f"Added {ticker}@{date_str} to unavailable cache (cache now has {len(unavailable_cache)} tickers)"
                )
            else:
...
def get_price_changes(
    ...
    for symbol in symbols:
        ...
        current_price, _ = get_price_from_yfinance(
            symbol, today_str, price_cache, unavailable_cache, split_cache
        )
        ...
        # Get 7-day ago price
        price_7d, _ = get_price_from_yfinance(
            symbol, date_7d, price_cache, unavailable_cache, split_cache
        )
        change_7d = None
        ...
        price_30d, _ = get_price_from_yfinance(
            symbol, date_30d, price_cache, unavailable_cache, split_cache
        )
        change_30d = None
...
def get_multi_period_returns(
    ...
    for symbol in symbols:
        ...
        for period_name, days in PERFORMANCE_PERIODS.items():
            ...
            period_price, _ = get_price_from_yfinance(
                symbol, period_date, price_cache, unavailable_cache, split_cache
            )
    ...
    if ticker not in split_cache:
        ...
        try:
            ...
            if splits is None or splits.empty:
                ...
                split_cache[ticker] = {
                    date.strftime("%Y-%m-%d"): float(ratio) for date, ratio in splits.items()
                }
        except Exception as e:
    ...
    for symbol in symbols_to_check:
        ...
        for idx in df[symbol_mask].index:
            ...
            if multiplier != 1.0:
                ...
                df.loc[idx, "Note"] = (
                    f"{existing_note}; {split_note}" if existing_note else split_note
                )
                adjusted_count += 1
...
def convert_price_based_symbols(df: pd.DataFrame, cache: dict) -> pd.DataFrame:
    ...
    for source_symbol, target_ticker in PRICE_CONVERSION_MAP.items():
        ...
        print(
            f"Converting {mask.sum()} transactions from '{source_symbol}' to '{target_ticker}'..."
        )
        ...
        for idx in df[mask].index:
            ...
            if price is not None and price > 0:
                ...
                df.loc[idx, "Symbol"] = target_ticker
                df.loc[idx, "Note"] = (
                    f"{df.loc[idx, 'Note']}; Converted from {source_symbol}"
                    if df.loc[idx, "Note"]
                    else f"Converted from {source_symbol}"
                )
            else:
                # Couldn't get price, just update symbol
                df.loc[idx, "Symbol"] = target_ticker
                df.loc[idx, "Note"] = (
                    f"{df.loc[idx, 'Note']}; Price lookup failed"
                    if df.loc[idx, "Note"]
                    else "Price lookup failed"
                )
