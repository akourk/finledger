...
This is the detailed engineering reference formerly kept in `CLAUDE.md`.
Start with [AGENTS.md](../AGENTS.md) and the
[development guide](../CONTRIBUTING.md); read the sections relevant to your
change, including their cross-consumer constraints. `src/main.py` defines
execution order; these notes explain the decisions and failure boundaries.

## How to read it

Read this reference on demand instead of loading it into every agent session.
Search its headings and relevant symbols with `rg -n` before opening a large
section. [Architecture](ARCHITECTURE.md) covers design decisions; the notes
here preserve **load-bearing rules, observed failures, and parallel consumers**.
Historical explanations describe the relevant bug class, not current account
state. Confirm named functions and tests against source when changing them.

Sections, in order:

| Section | What it is for |
| --- | --- |
| [Project and execution](#what-this-project-is) | Orientation; use CONTRIBUTING for development commands. |
| [Pipeline](#pipeline) | Stage semantics, analytics payloads, dashboard consumers. |
| [Ledger rules](#invariants-the-code-relies-on) | Signs, action semantics, metadata, cash flow, account classes. |
| [Broker](#adding-a-new-broker) / [action](#adding-a-new-normalized-action) | Extension routes to maintained skills. |
| [Module map](#architecture-post-refactor) / [test resets](#module-reset-functions-for-tests) | Shared helpers and their consumers. |
| [Cost basis](#invariants-the-cost-basis-walker-relies-on) / [prices](#invariants-the-price-cache-relies-on) | Financial invariants and parity checks. |
| [Local caches](#local-cache-and-metadata-files) / [repair](#repairing-a-suspect-price-cache) / [snapshots](#snapshot-feature) | Operational boundaries. |
| [Privacy](#data-is-sensitive) / [sample data](#adding-sample-data) | Public artifacts and fictional input provenance. |
...
- **Record the failure, not just the rule.**  "Do X" decays into
  ...
  Performance tab disagree" survives a refactor that changes the
  shape of the code.
...
[`docs/AUDIT.md`](AUDIT.md), the sanitized record of a systematic
defect hunt; the method behind it is in
[`docs/PLAN-audit.md`](PLAN-audit.md).
...
The app is organized into Python modules under `src/`, including
the `parsers/`, `analytics/`, and `dashboard/` packages plus top-level
pipeline modules — with **`yfinance` as the direct Python runtime dependency**
...
calls). Development dependencies are declared in `pyproject.toml` and
`package.json`; the generated dashboard has no external runtime assets. See
the module map below and [README.md](../README.md) for orientation.

## Run it

Use [CONTRIBUTING.md](../CONTRIBUTING.md#development-loop) for isolated
development and [USAGE.md](USAGE.md) for normal local operation. The default
`src.main` command operates on personal paths; it is not a development smoke
test. Generate reviewable dashboards with the dedicated `tools.build_demo`
builder, which isolates all paths, fixes the date, and disables network access.
Normal local output lives in `exports/transactions.json` and
...
2. **Rename** (`scanner.rename_data_files`) — two-pass rename to
   `{broker-prefix}.csv` or `{broker-prefix}-{n}.csv`. A unique
   `.fin-rename-*` staging directory avoids collisions with inputs and stale
   intermediates. Both phases roll back on failure; if rollback itself fails,
   preserve the recovery files. Scan, parse, and rename refuse a pending
   recovery directory, and legacy `.tmp-*` inputs are reported as unknown
   rather than silently skipped. See [recovery](USAGE.md#input-validation-and-recovery).
   Scanner-`skip`
...
3. **Parse** (`parsers.parse_all_files`) — dispatch to a per-broker parser.
   ...
   `validate_ingestion` rejects unrecognized inputs, dropped rows, empty imports,
   and non-finite values before export; a partial parse must not replace the
   previous dashboard. Shared `read_csv_rows` preserves physical row locations.
...
7. **Account tagging** — `accounts.configure_accounts` loads metadata, clears
   and updates `ACCOUNT_GROUPS` / `ACCOUNT_TYPES`, and tags each transaction.
   Every resulting group requires an explicit `Taxable`, `Retirement`, or
   `Savings` type; a missing type blocks the run before output replacement.
...
10. **Holdings** — aggregate final non-zero balances per symbol (and per
    ...
    - if we have a price: drop if resolved dollar value is below a cent
    - if we don't: drop if `|qty| < 1e-6`
    Use `valuation.mark_is_dust` for a resolved mark: its dollars already
    include split restatement and the option contract multiplier. Recomputing
    from raw quantity and unit price can discard a material option or a
    historical split-adjusted position. Retain unrounded unit prices as
    calculation inputs; display formatting must not change valuation or P&L.
...
13c. **Derived analytics** (`analytics.build_analytics`) — single source
    ...
    - `monte_carlo` — `{retirement, all_accounts, fi_threshold,
      ...
      × 25` (4% rule).
      Contribution pace comes from `monte_carlo.trailing_annual_contribution`:
      change in cumulative contributions divided by elapsed calendar time,
...
- **Browser calendar dates are dates, not local-time durations.** Use
  `shiftCalendarIso` for day/month/year window boundaries, including DST and
  leap-year handling, and `calendarAge` for the Python-compatible birthday
  rule. The dashboard stays pinned to its exported snapshot date.
  Trailing retirement contributions use the shared signed helper in
  `app/50-retirement.js`; contribution reversals subtract, and transactions
  after the snapshot date do not enter the total. Planning and income
  forecast defaults consume the same result. Covered by
  `tests/test_frontend_review_regressions.py`.
...
- **`metadata.csv` is user-maintained metadata + project config.** The
  ...
  `accounts.configure_accounts` applies and validates them in both pipeline
  paths**. Calling `parse_metadata` alone
  ...
  - `Filing Status` — Note = `Single` / `Married Filing Jointly` /
    ...
    **single source of truth**: `tax_tables_to_json()` serializes
    ...
    in `app/80-tax.js` are emergency fallbacks only). Adding a new tax year
  ...
  - `Tax Return` — a figure from a FILED 1040.  Date = tax year,
    ...
    **safe-harbor check** (`rate_estimates_by_year[current].safe_harbor`):
    ...
    `ACCOUNT_TYPES` also starts empty in config.py. Normal pipeline runs
    require explicit types; isolated helper fallbacks do not bypass that gate.
...
- **Section 1256 underlyings are single-sourced from
  ...
  them from there (the `app/80-tax.js` literal is an emergency fallback only).
...
Use [fin-add-broker](../.claude/skills/fin-add-broker/SKILL.md) for detection,
strict CSV helpers, account mappings, dispatch, normalization, and fictional
snapshot coverage. Direction is resolved before absolute-value conversion;
missing account types and rejected transaction rows must block export.

## Adding a new normalized action

Use [fin-add-action](../.claude/skills/fin-add-action/SKILL.md). The catalog in
`src/actions.py` owns balance, basis, cash-flow, income, and color fields;
consumer sets derive from it. A new basis effect still requires corresponding
walker behavior, and broker cash-flow exceptions remain in the shared helper.
JavaScript reads the per-transaction exported `cash_flow` verdict, which
includes those exceptions, rather than reconstructing it from catalog sets.
...
- **`src/broker_lots.py`** — report-directed lot relief.  Parses the
  ...
  **wallet-move no-op** (`_rebase_is_move`) that re-pushes the consumed
  ...
  convert; (2) **a broker report can book an internal wallet arrival as a second
  acquisition without removing the source inventory**. Its later disposal can
  then name a lot that the physical ledger already consumed. No consume-order
  strategy can reproduce inventory absent from the ledger. Do not force an
  offsetting per-year timing difference to disappear by changing lot order;
  compare the report's inventory semantics first. This is a data limitation,
...
- **`tests/`** — pytest regression suite.  Covers:
  ...
  Run with `uv run python -m pytest tests/ -q -rs`.  All tests must pass before
  merging changes.
...
- **`basis_totals` is the export's authoritative portfolio-level
  ...
  `basis_methods.fifo` made Overview and Performance report different values
  for the same realized-gain quantity (docs/AUDIT.md F-033); the two tabs then
...
- **The store is DAILY.** An intraday series would need a demonstrated
  consumer, a supported historical-data source, and an explicit storage model.
  Existing price consumers are daily-or-coarser (`history.compute_history`,
...
- **NaN closes are never cached or returned.** yfinance emits `NaN` for
  ...
  default Python JSON encoding permits a bare `NaN` literal. The export
  now uses `allow_nan=False` as another boundary, so non-finite results stop
  publication instead of reaching dashboard totals. Keep both cache validation
  and strict export; do not introduce `NaN` in an isolated cache fixture except
  to test rejection.
...
<a id="local-cache-and-metadata-files"></a>
...
- `cache/sector_cache.json` and `cache/prices/{SYMBOL}.json` — both live.
  ...
  authoritative, not the filename). Preserve local history when repairing
  coverage: delisted symbols may no longer be fetchable. Use the repair
  workflow below and test cache changes on isolated fictional copies.
...
scan steps in [docs/PRIVACY.md](PRIVACY.md).
...
mostly index funds + a bit of crypto). The generator uses a fixed sample date.
After parser changes, validate `test_sample_snapshot.py` and the isolated demo;
regenerate only for an intentional fixture update or an understood stale-sample
failure. The per-broker writers double as CSV format documentation.
